...
public class MetaKnight extends Character {
	...
	public MetaKnight()
	{
		...
		super("MetaKnight", 2, 15, 5, new Rectangle(64, 65), new Rectangle(40, 65), new Rectangle(128, 80), 1, 7, 10, 3);
		...
		try
		{
			sprites.add(ImageIO.read(new File("MetaKnightR.png")));
			sprites.add(ImageIO.read(new File("MetaKnightL.png")));
			
			// Walking
			for(int i = 0; i < 10; i++)
			{
				walkRSprites.add(ImageIO.read(new File("MetaKnightRWalk" + (i+1) + ".png")));
				walkLSprites.add(ImageIO.read(new File("MetaKnightLWalk" + (i+1) + ".png")));
			}
			
			// Attacking
			for(int i = 0; i < 7; i++)
			{
				attackRSprites.add(ImageIO.read(new File("MetaKnightRAttack" + (i+1) + ".png")));
				attackLSprites.add(ImageIO.read(new File("MetaKnightLAttack" + (i+1) + ".png")));
			}
			
			// Special Attack
			for(int i = 0; i < 9; i++)
			{
				specialRSprites.add(ImageIO.read(new File("MetaKnightRSpecial" + (i+1) + ".png")));
				specialLSprites.add(ImageIO.read(new File("MetaKnightLSpecial" + (i+1) + ".png")));
			}
		}
		catch (Exception e)
		{
