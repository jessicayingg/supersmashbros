...
public class Luigi extends Character {
	...
	public Luigi()
	{
		...
		try
		{
			sprites.add(ImageIO.read(new File("LuigiR.png")));
			sprites.add(ImageIO.read(new File("LuigiL.png")));
			
			// Walking
			for(int i = 0; i < 8; i++)
			{
				walkRSprites.add(ImageIO.read(new File("LuigiRWalk" + (i+1) + ".png")));
				walkLSprites.add(ImageIO.read(new File("LuigiLWalk" + (i+1) + ".png")));
			}
			
			// Attacking
			for(int i = 0; i < 4; i++)
			{
				attackRSprites.add(ImageIO.read(new File("LuigiRAttack" + (i+1) + ".png")));
				attackLSprites.add(ImageIO.read(new File("LuigiLAttack" + (i+1) + ".png")));
			}
			
			// Special Attack
			for(int i = 0; i < 6; i++)
			{
				specialRSprites.add(ImageIO.read(new File("LuigiRSpecial" + (i+1) + ".png")));
				specialLSprites.add(ImageIO.read(new File("LuigiLSpecial" + (i+1) + ".png")));
			}
