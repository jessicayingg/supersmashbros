...
public class Kirby extends Character {
	...
	public Kirby()
	{
		...
		try
		{
			sprites.add(ImageIO.read(new File("KirbyR.png")));
			sprites.add(ImageIO.read(new File("KirbyL.png")));
			
			// Walking
			for(int i = 0; i < 10; i++)
			{
				walkRSprites.add(ImageIO.read(new File("KirbyRWalk" + (i+1) + ".png")));
				walkLSprites.add(ImageIO.read(new File("KirbyLWalk" + (i+1) + ".png")));
			}
			
			// Attacking
			for(int i = 0; i < 8; i++)
			{
				attackRSprites.add(ImageIO.read(new File("KirbyRAttack" + (i+1) + ".png")));
				attackLSprites.add(ImageIO.read(new File("KirbyLAttack" + (i+1) + ".png")));
			}
			
			// Special Attack
			for(int i = 0; i < 4; i++)
			{
				specialRSprites.add(ImageIO.read(new File("KirbyRSpecial" + (i+1) + ".png")));
				specialLSprites.add(ImageIO.read(new File("KirbyLSpecial" + (i+1) + ".png")));
			}
		}
		catch (Exception e)
		{
