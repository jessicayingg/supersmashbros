...
public class Link extends Character {
	...
	public Link()
	{
		...
		try
		{
			sprites.add(ImageIO.read(new File("LinkR.png")));
			sprites.add(ImageIO.read(new File("LinkL.png")));
			
			// Walking
			for(int i = 0; i < 10; i++)
			{
				walkRSprites.add(ImageIO.read(new File("LinkRWalk" + (i+1) + ".png")));
				walkLSprites.add(ImageIO.read(new File("LinkLWalk" + (i+1) + ".png")));
			}
			
			// Attacking
			for(int i = 0; i < 5; i++)
			{
				attackRSprites.add(ImageIO.read(new File("LinkRAttack" + (i+1) + ".png")));
				attackLSprites.add(ImageIO.read(new File("LinkLAttack" + (i+1) + ".png")));
			}
			
			specialRSprite.add(ImageIO.read(new File("LinkRSpecial.png")));
			specialLSprite.add(ImageIO.read(new File("LinkLSpecial.png")));
