...
public class Main extends JPanel implements ActionListener, KeyListener, Runnable {
	...
	public static Rectangle[] stage1 = {new Rectangle(182, 288, 185, 30), new Rectangle(587, 288, 185, 30), new Rectangle(384, 184, 185, 30)};
	public static Rectangle[] stage2 = {new Rectangle(422, 288, 185, 30), new Rectangle(121, 184, 185, 30), new Rectangle(306, 184, 185, 30)};
	...
	// Invulnerability Variables (you are invulnerable for a little bit after respawning)
	...
	public static boolean changingUpKey, changingDownKey, changingLeftKey, changingRightKey, changingAttackKey, changingSpecialKey;
	...
	public Main()
	{
		...
		{
			// Adding images
			titleScreen = ImageIO.read(new File("SuperSmashBros.jpeg"));
			for(int i = 0; i < 5; i++)
			{
				mainImages[i] = ImageIO.read(new File("SmashMain" + (i+1) + ".png"));
				settingsLifeCountImages[i] = ImageIO.read(new File("SettingsLives" + (i+1) + ".png"));
			}
			for(int i = 0; i < 2; i++)
			{
				for(int j = 0; j < 4; j++)
				{
					selectImages[i][j] = ImageIO.read(new File("P" + (i+1) + "Select" + (j+1) + ".png"));
				}
			}
			
			for(int i = 0; i < 4; i++)
			{
				characterSelectImages[i] = ImageIO.read(new File("COption" + (i+1) + ".png"));
				inGameIcons[0][i] = ImageIO.read(new File("P1InGameIcon" + (i+1) + ".png"));
				inGameIcons[1][i] = ImageIO.read(new File("P2InGameIcon" + (i+1) + ".png"));
				specificStatsImages[i] = ImageIO.read(new File("StatsDetails" + (i+1) + ".png"));
				highestCharIcons[i] = ImageIO.read(new File("StatsWin" + (i+1) + ".png"));
				settingsTimeImages[i] = ImageIO.read(new File("SettingsTime" + (i+1) + ".png"));
				winnerScreenImages[0][i] = ImageIO.read(new File("P1Victory" + (i+1) + ".png"));
				winnerScreenImages[1][i] = ImageIO.read(new File("P2Victory" + (i+1) + ".png"));
				loserImages[i] = ImageIO.read(new File("Loser" + (i+1) + ".png"));
			}
			
			generalStatsImage = ImageIO.read(new File("StatsOverview.png"));
			
			instructionsImages[0] = ImageIO.read(new File("Instructions1.png"));
			instructionsImages[1] = ImageIO.read(new File("Instructions2.png"));
			
			aboutImage = ImageIO.read(new File("About.png"));
			
			for(int i = 0; i < 3; i++)
			{
				settingsSelectImages[i] = ImageIO.read(new File("SettingsMain" + (i+1) + ".png"));
			}
			settingsKeyChangeImages[0] = ImageIO.read(new File("SettingsP1KeyChange.png"));
			settingsKeyChangeImages[1] = ImageIO.read(new File("SettingsP2KeyChange.png"));
			
			mapSelectImages[0] = ImageIO.read(new File("MapSelect1.png"));
			mapSelectImages[1] = ImageIO.read(new File("MapSelect2.png"));
			
			pauseSelectImages[0] = ImageIO.read(new File("Pause1.png"));
			pauseSelectImages[1] = ImageIO.read(new File("Pause2.png"));
			
			respawnPlatform = ImageIO.read(new File("RespawnPlatform.png"));
			gameEnd = ImageIO.read(new File("GameEnd.png"));
			pausedCover = ImageIO.read(new File("PausedCover.png"));
			
			livesLeftCounterImages[0] = ImageIO.read(new File("LifeCounterL.png"));
			livesLeftCounterImages[1] = ImageIO.read(new File("LifeCounterR.png"));
			
			tieImage = ImageIO.read(new File("NoVictory.png"));
			
			// Adding music
			AudioInputStream sound = AudioSystem.getAudioInputStream(new File ("ThemeSong.wav"));
			gameSong = AudioSystem.getClip();
			gameSong.open(sound);
			
			sound = AudioSystem.getAudioInputStream(new File ("MainSong.wav"));
	...
	public void paintComponent(Graphics g)
	{
		...
		else if(gameState == 3)
		{
			...
			// Using small pale yellow circles underneath each character icon to show number of
			// lives remaining
			...
			else if(!p1Special)
			{
				...
				if(p1FacingLeft)
				{
					g.drawImage(p1.getSprites("attackL").get(p1AttackImage), p1X - (int) p1.getAttackBox().getWidth(), p1Y - (p1.getAttackingHeight()-p1.getHeight()), p1.getAttackingWidth(), p1.getAttackingHeight(), this);
				}
				// Facing right
				else
				{
					g.drawImage(p1.getSprites("attackR").get(p1AttackImage), p1X, p1Y - (p1.getAttackingHeight()-p1.getHeight()), p1.getAttackingWidth(), p1.getAttackingHeight(), this);
				}
			...
			else if(!p2Special)
			{
				...
				if(p2FacingLeft)
				{
					g.drawImage(p2.getSprites("attackL").get(p2AttackImage), p2X - (int) p2.getAttackBox().getWidth(), p2Y - (p2.getAttackingHeight()-p2.getHeight()), p2.getAttackingWidth(), p2.getAttackingHeight(), this);
				}
				// Facing right
				else
				{
					g.drawImage(p2.getSprites("attackR").get(p2AttackImage), p2X, p2Y - (p2.getAttackingHeight()-p2.getHeight()), p2.getAttackingWidth(), p2.getAttackingHeight(), this);
				}
			...
			// Drawing a respawn platform and making the screen darker when a winner is decided
			if(winner != 0)
			{
				g.drawImage(respawnPlatform, 0, 0, 960, 540, this);
	...
	// Description: This is a run method responsible all actions that have to be constantly updated
	// Parameters: None
	// Return: void
	public void run() 
	{
		...
		else if(gameState == 2)
		{
			...
			else if(key == 10)
			{
				...
				else
				{
					...
					if(optionSelected == 0)
					{
						selectedStage = new Stage("Stage1.png", groundBox, stage1[0], stage1[1], stage1[2]);
					}
					// Second stage option
					else
					{
						selectedStage = new Stage("Stage2.png", groundBox, stage2[0], stage2[1], stage2[2]);
					}
	...
	// Description: This facilitates the increase/decrease of a character's x speed, x value, etc.
		...
		if(!paused && winner == 0)
		{
			if(gameState == 3)
			{
				if(!p1KO)
				{
					...
					if(p1.getHitBox().getMaxX() < p1CurLedge.getMinX() || p1.getHitBox().getMinX() < p1CurLedge.getMaxX())
					{
						if(!p1KO)
						{
							p1Airborne = true;
				...
				if(!p2KO)
				{
					...
					// If you are on a ledge and you fall off
					if(p2.getHitBox().getMaxX() < p2CurLedge.getMinX() || p2.getHitBox().getMinX() < p2CurLedge.getMaxX())
					{
						if(!p2KO)
						{
							p2Airborne = true;
	...
	// Description: This helps to keep characters within the set bounds (bottom, left, right. etc.)
	...
	// Description: This method facilitates the switching between images to create a walking
	// animation
	...
	// Description: This method facilitates the switching between images to create a walking
	// animation
	// Parameters: A String that indicates the player number that is playing, an int that represents
	...
	// Description: This method is used for respawning characters after they've been KO'ed
	// Parameters: None
	// Return: Void
	...
	// Description: This method deals with a character's invulnerability right after respawning
	// Parameters: None
	// Return: Void
	...
	// Description: This adds all the images for the keys that appear in the key mapping settings
	// Paramters: None
	// Return: Void
	public static void loadKeyImages()
	{
		...
		try
		{
			keyImageMap.put(32, ImageIO.read(new File("32.png")));
			
			for(int i = 37; i < 41; i++)
			{
				keyImageMap.put(i, ImageIO.read(new File(i + ".png")));
			}
			for(int i = 44; i < 48; i++)
			{
				keyImageMap.put(i, ImageIO.read(new File(i + ".png")));
			...
			keyImageMap.put(59, ImageIO.read(new File("59.png")));
			keyImageMap.put(61, ImageIO.read(new File("61.png")));
			...
			keyImageMap.put(222, ImageIO.read(new File("222.png")));
	...
	// Description: This method updates the timer and if the timer is depleted, determines the winner
	// Parameters: None
	// Return: Void
	...
	// Description: This method updates the number of times played and number of wins for the characters
	// Parameters: None
	// Return: Void
	...
	public static void readInStats()
	{
		...
		try
		{
			BufferedReader inFile = new BufferedReader(new FileReader("Stats.txt"));
	...
	public static void writeInStats()
	{
		try
		{
			PrintWriter outFile = new PrintWriter(new FileWriter("Stats.txt"));
	...
	// Description: This method looks for the character(s) with the most wins and most times played
	// Parameters: None
	// Return: Void
	...
	// Description: This method finds the number of wins and number of times played for a specific character
